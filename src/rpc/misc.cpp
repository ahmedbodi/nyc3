...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given nyc3 address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The nyc3 address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The nyc3 address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Nyc3 address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are nyc3 addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) nyc3 address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The nyc3 address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "nyc3-cli and the GUI. There is no server-side difference."
        );
...
{
    if (type == 2) {
        address = CNyc3Address(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CNyc3Address(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CNyc3Address address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        if (!addressValues.isArray()) {
         ...
         for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
             ...
             CNyc3Address address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
