...
The RPM spec file provided here is for Nyc3-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://nyc3.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Nyc3 git repository](https://github.com/Nyc3Project/Nyc3coin) but are not part of
...
Currently only `Source10` is used, for the example `nyc3.conf` file.
...
in the nyc3 git repository. At present only an SVG version of the Nyc3
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the nyc3
...
needed when 0.12.1 is released, a proper fix is already in the Nyc3 git
master branch.
...
idea to build Nyc3 Core against a static version of that library compiled
...
The `nyc3.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `nyc3-server` package. At this point, most RPM based
...
The `nyc3-server` package should have SELinux support. How to properly do
...
Nyc3-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/nyc3/). He has been
packaging Nyc3 for Fedora at least since 2012.
...
4. I renamed the `nyc3` package that contains the Qt GUI to `nyc3-core` as
that appears to be how the general population refers to it, in contrast to
`nyc3-xt` or `nyc3-classic`. I wanted to make sure the general population
